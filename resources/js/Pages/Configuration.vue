        ...
        <ContentLayout>
            ...
            <Modal v-if="isEditModalShown" @close="toggleEditModal">
              <template v-slot:title>
                    <p class="font-bold text-xl">Edit Configuration</p>
                </template>
                <template v-slot:body>
                    <form action="" class="mt-2">
                        <div class="flex justify-center mt-2">
                            <div class="w-36 rounded h-24  border-2 border-gray-400 flex items-center justify-center shadow-md overflow-hidden">
                                <img v-if="modal_data.image" :src="`images/car-images/${modal_data.image}`" class="object-fit  h-full" alt="" />
                            </div>
                        </div>
                        ...
                        <div>
                         ...
                         <div class="container flex flex-col px-4 space-y-5 justify-center mt-10">
                            ...
                            <FormInput id="name" v-model="modal_data.name" name="name" type="text" placeholder="Enter name" label="Name" :error="errors.name"/>

                            <FormInput id="fee" v-model="modal_data.fee" name="fee" type="number" placeholder="Enter fee" label="Fee" :error="errors.fee"/>

                             <FormTextArea id="details" v-model="modal_data.details" placeholder="Enter details" label="Details" :error="errors.details"/>
                ...
                <template v-slot:footer >
                  <div class="space-x-4 ">
                      <button type="submit" class="px-3 py-2 border-2 border-black bg-white-700 text-black rounded-md text-base tracking-wider uppercase font-medium  hover:cursor-pointer mt-2" size="sm" @click.prevent="cancelEditModal" :disabled="form.processing">Cancel</button>
